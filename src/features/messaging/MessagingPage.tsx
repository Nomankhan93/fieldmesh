...
export function MessagingPage() {
  ...
  const queuedCount = useLiveQuery(
    () => localUserId ? db.cloudOutbox.where('localUserId').equals(localUserId).count() : Promise.resolve(0),
    [localUserId],
    0,
  )
  ...
  useEffect(() => {
    void refresh().catch((error) => setNotice(error instanceof Error ? error.message : 'Unable to synchronize messages.'))
  }, [refresh])

  useEffect(() => {
    if (!service || !localUserId) return
    const timer = window.setInterval(() => void refresh().catch(() => undefined), 3_000)
    const handleOnline = () => { setBrowserOnline(true); void refresh().catch(() => undefined) }
    const handleOffline = () => setBrowserOnline(false)
    const handleVisibility = () => { if (document.visibilityState === 'visible') void refresh().catch(() => undefined) }
    ...
    <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      <header className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Direct and group conversations use the same durable mailbox. Transport and security internals stay out of the normal chat flow.</p>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
        <aside className={`space-y-4 ${selectedConversationId ? 'hidden lg:block' : 'block'}`}>
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {composerMode === 'direct' ? (
              <form className="mt-4 space-y-2" onSubmit={createDirectConversation}>
                ...
                <button className="w-full rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50" disabled={busy}>Start chat</button>
              </form>
            ) : (
              <form className="mt-4 space-y-2" onSubmit={createGroup}>
                ...
                <button className="w-full rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50" disabled={busy || !groupTitle.trim()}>Create group</button>
              </form>
            )}
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Conversations</h2><button type="button" className="text-xs font-semibold text-slate-600" onClick={() => void refresh()}>Sync</button></div></div>
            <div className="divide-y divide-slate-100">
              {conversations.length === 0 ? (
                <div className="p-5 text-center"><div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-lg" aria-hidden="true">✦</div><p className="mt-3 text-sm font-semibold text-slate-700">No conversations yet</p><p className="mt-1 text-xs leading-5 text-slate-500">Start a direct chat or create your first group.</p></div>
          ...
          {selectedConversationId && selectedConversation ? (
            <>
              ...
              {selectedConversation.kind === 'group' && showGroupManager ? (
                <GroupManagementPanel conversation={selectedConversation} participants={selectedParticipants} localUserId={localUserId} service={messagingService} onChanged={refresh} onLeft={() => setSelectedConversationId(null)} setNotice={setNotice} />
              ) : null}
